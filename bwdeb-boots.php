...
class BWDEBClassElementorBundle {
	...
	public function bwdeb_admin_editor_scripts() {
		wp_enqueue_script( 'bwdeb_the_contact_form_editor', plugin_dir_url( __FILE__ ) . 'assets/public/js/empty-dummy.js', array(), '3.2.6', true );
	...
	public function bwdeb_all_assets_for_the_public(){
		...
		foreach($script_paths as $filepath){
			wp_enqueue_script( 'bwdcssk-content-'.$filepath, plugin_dir_url( __FILE__ ) . 'assets/public/js/'.$filepath.'.js', array('jquery'), '3.2.6', true );
		}
		...
		foreach($stylefile as $filepath){
			wp_enqueue_style( 'bwdeb-'.$filepath, plugin_dir_url( __FILE__ ) . 'assets/public/css/'.$filepath.'.css', null, '3.2.6', 'all' );
		}
	...
	public function bwdeb_all_assets_for_elementor_editor_admin(){
      ...
      wp_enqueue_style( $handle, $fileinfo['bwdeb_path_admin_define'], null, '3.2.6', 'all');
		}
	}
	...
	public function bwdeb_all_assets_for_dashboard_admin(){
		...
		foreach($adCsAss as $pathCs){
			wp_enqueue_style( 'bwdeb-'.$pathCs,  plugin_dir_url( __FILE__ ) . 'assets/admin/'.$pathCs.'.css', null, '3.2.6', 'all' );
		}
		$adJsAss = ['dashboard/main', 'dashboard/widgets-cnt', 'js/checkbox-status'];
		foreach($adJsAss as $pathJs){
			wp_enqueue_script( 'bwdeb-'.$pathJs,  plugin_dir_url( __FILE__ ) . 'assets/admin/'.$pathJs.'.js', ['jquery'], '3.2.6', true );
		}
	}
