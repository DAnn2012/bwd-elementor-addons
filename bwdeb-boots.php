...
class BWDEBClassElementorBundle {
	...
	public function bwdeb_all_assets_for_the_public(){
		...
		foreach($script_paths as $filepath){
			wp_enqueue_script( 'bwdcssk-content-'.$filepath, plugin_dir_url( __FILE__ ) . 'assets/public/js/'.$filepath.'.js', array('jquery'), '2.2.0', true );
		}
		...
		foreach($stylefile as $filepath){
			wp_enqueue_style( 'bwdeb-'.$filepath, plugin_dir_url( __FILE__ ) . 'assets/public/css/'.$filepath.'.css', null, '2.2.0', 'all' );
		}
	}
	public function bwdeb_all_assets_for_elementor_editor_admin(){
      ...
      wp_enqueue_style( $handle, $fileinfo['bwdeb_path_admin_define'], null, '2.2.0', 'all');
		}
	}
	public function bwdeb_all_assets_for_dashboard_admin(){
		wp_enqueue_style( 'bwdeb-dashboard-admin',  plugin_dir_url( __FILE__ ) . 'assets/admin/css/admin-basic.css', null, '2.2.0', 'all' );
		// For dashboard
		wp_enqueue_style( 'bwdeb-index-dashboard-admin',  plugin_dir_url( __FILE__ ) . 'assets/admin/dashboard/style.css', null, '2.2.0', 'all' );
		wp_enqueue_script( 'bwdeb-index-dashboard-admin-script',  plugin_dir_url( __FILE__ ) . 'assets/admin/dashboard/main.js', ['jquery'], '2.2.0', true );
		// For Check Box Status
		wp_enqueue_script( 'bwdeb-index-dashboard-checkbox-status',  plugin_dir_url( __FILE__ ) . 'assets/admin/js/checkbox-status.js', ['jquery'], '2.2.0', true );
	}
