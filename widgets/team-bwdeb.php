...
class BWDEB_MTTMeetTheTeam extends Widget_Base {
	...
	protected function register_controls() {
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .mtt_gapc' => 'padding-left: {{SIZE}}{{UNIT}}; padding-right: {{SIZE}}{{UNIT}};',
					'{{WRAPPER}} .mtt_gapr' => 'margin-left: -{{SIZE}}{{UNIT}}; margin-right: -{{SIZE}}{{UNIT}};',
				],
			]
		);
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .mtt-total-team-box' => 'margin-bottom: {{SIZE}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					...
					'{{WRAPPER}} .mtt-social-icon-one svg' => 'width: {{SIZE}}{{UNIT}};',
					'{{WRAPPER}} .mtt-social-icon-two svg' => 'width: {{SIZE}}{{UNIT}};',
					'{{WRAPPER}} .mtt-social-icon-three svg' => 'width: {{SIZE}}{{UNIT}};',
					'{{WRAPPER}} .mtt-social-icon-four svg' => 'width: {{SIZE}}{{UNIT}};',
				],
			]
		);
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .mtt-social a' => 'width: {{SIZE}}{{UNIT}}!important; height: {{SIZE}}{{UNIT}}!important;',
				],
			]
		);
		$this->end_controls_section();
		...
		$this->add_responsive_control(
            ...
            [
                ...
                'size_units' => ['px', 'em', '%'],
                'selectors' => [
                    '{{WRAPPER}} .mtt-total-team-box' => 'margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
                ],
            ]
        );
		$this->add_responsive_control(
            ...
            [
                ...
                'size_units' => ['px', 'em', '%'],
                'selectors' => [
                    '{{WRAPPER}} .mtt-total-team-box' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
                ],
            ]
        );
		$this->add_group_control(
	...
	protected function render() {
		...
		if ( ! empty( $settings['mtt_social_icon_one_link']['url'] ) ) {
			$this->add_link_attributes( 'mtt_social_icon_one_link', esc_url($settings['mtt_social_icon_one_link']) );
		}
		if ( ! empty( $settings['mtt_social_icon_two_link']['url'] ) ) {
			$this->add_link_attributes( 'mtt_social_icon_two_link', esc_url($settings['mtt_social_icon_two_link']) );
		}
		if ( ! empty( $settings['mtt_social_icon_three_link']['url'] ) ) {
			$this->add_link_attributes( 'mtt_social_icon_three_link', esc_url($settings['mtt_social_icon_three_link']) );
		}
		if ( ! empty( $settings['mtt_social_icon_four_link']['url'] ) ) {
			$this->add_link_attributes( 'mtt_social_icon_four_link', esc_url($settings['mtt_social_icon_four_link']) );
		}
			...
			if('style1' === $settings['mtt_style_selection']){
				...
				<div class="mtt-team-area">
					<div class="container-fluid">
						<div class="row mtt_gapr <?php echo esc_attr($column_align); ?>">
					<?php
						if ( $settings['mtt_box'] ) {
							foreach (  $settings['mtt_box'] as $item ) {
						echo '<div class="column_number mtt_gapc col-xl-3 col-lg-4 col-md-4 col-sm-6 elementor-repeater-item-' . esc_attr( $item['_id'] ) . '">'; ?>
								...
								<div class="mtt-our-team-item">
									...
									<div class="mtt-team-content mtt_team_profile_background">
										...
										<div class="mtt-social">
											...
											<?php if('yes' === $item['mtt_social_icon_two_switcher']){ ?><a class="mtt-social-icon-two" href="<?php echo esc_url($item['mtt_social_icon_two_link']['url']); ?>"><?php \Elementor\Icons_Manager::render_icon( $item['mtt_social_icon_two'], [ 'aria-hidden' => 'true' ] ); ?></a> <?php } ?>
											<?php if('yes' === $item['mtt_social_icon_three_switcher']){ ?><a class="mtt-social-icon-three" href="<?php echo esc_url($item['mtt_social_icon_three_link']['url']); ?>"><?php \Elementor\Icons_Manager::render_icon( $item['mtt_social_icon_three'], [ 'aria-hidden' => 'true' ] ); ?></a> <?php } ?>
											<?php if('yes' === $item['mtt_social_icon_four_switcher']){ ?><a class="mtt-social-icon-two" href="<?php echo esc_url($item['mtt_social_icon_four_link']['url']); ?>"><?php \Elementor\Icons_Manager::render_icon( $item['mtt_social_icon_four'], [ 'aria-hidden' => 'true' ] ); ?></a> <?php } ?>
										</div>
									</div>
								</div>
							</div>
