...
class BWDEB_ptnticker_newsticker_widgets extends Widget_Base {
	...
	protected function register_controls() {
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .ptnticker-width' => 'width: {{SIZE}}{{UNIT}};',
				],
				'condition' => [
					'ptnticker_ticker_sticky_switcher' => 'yes',
				],
		...
		$this->add_control(
			...
			[
				'label' => esc_html__( 'Tag Text', 'bwd-elementor-addons' ),
				'type' => Controls_Manager::TEXT,
				'default' => esc_html__('Post Type News Ticker', 'bwd-elementor-addons'),
				'label_block' => true,
				'condition' => [
		...
		$this->start_controls_section(
			...
			[
				'label' => esc_html__( 'Query', 'bwd-elementor-addons' ),
				'tab' => Controls_Manager::TAB_CONTENT,
			]
		);
		// For Author Filter
		$ptnticker_array = get_users();
		foreach ($ptnticker_array as $ptnticker_author){
			$ptnticker_id = $ptnticker_author->ID;
			$ptnticker_name = $ptnticker_author->display_name;
			$ptnticker_get_array[$ptnticker_id] = $ptnticker_name;
		}
		$this->add_control(
		...
		// For Category Filter
		$ptnticker_idObj = get_category_by_slug('category-slug');
		$ptnticker_order_options = array($ptnticker_idObj => 'All Categories');
		$ptnticker_categories = get_categories('orderby=name&hide_empty=0');
		foreach ($ptnticker_categories as $category):
			$ptnticker_catids = $category->slug;
			$ptnticker_catname = $category->name;
			$ptnticker_order_options[$ptnticker_catids] = $ptnticker_catname;
		endforeach;
		$this->add_control(
			'ptnticker_the_cat_columnsdd',
			[
				'label' => esc_html__( 'Filter Categories', 'bwd-elementor-addons' ),
				'type' => Controls_Manager::SELECT,
				'default' => 0,
				'options' => $ptnticker_order_options,
			]
		);
		// For tag Filter
		$ptnticker_idObj = get_tag('tags-slug');
		$ptnticker_order_optionst = array($ptnticker_idObj => 'All Tags');
		$ptnticker_ta = get_tags('orderby=name&hide_empty=0');
		foreach ($ptnticker_ta as $tag):
			$ptnticker_tagids = $tag->slug;
			$ptnticker_tagname = $tag->name;
			$ptnticker_order_optionst[$ptnticker_tagids] = $ptnticker_tagname;
		endforeach;
		$this->add_control(
			'ptnticker_the_tag_filter',
			[
				'label' => esc_html__( 'Filter Tags', 'bwd-elementor-addons' ),
				'type' => Controls_Manager::SELECT,
				'default' => $ptnticker_idObj,
				'options' => $ptnticker_order_optionst,
			]
		);
		$this->end_controls_section();
		...
		$this->add_responsive_control(
            ...
            [
                ...
                'size_units' => ['px', 'rem', '%'],
                'selectors' => [
                    '{{WRAPPER}} .ptnticker-title' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
                ],
            ]
        );
		$this->add_responsive_control(
            ...
            [
                ...
                'size_units' => ['px', 'rem', '%'],
                'selectors' => [
                    '{{WRAPPER}} .ptnticker-title' => 'margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
                ],
            ]
        );
		...
		$this->add_responsive_control(
            ...
            [
                ...
                'size_units' => ['px', 'rem', '%'],
                'selectors' => [
                    '{{WRAPPER}} .ptnticker-items' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
                ],
            ]
        );
		$this->add_responsive_control(
            ...
            [
                ...
                'size_units' => ['px', 'rem', '%'],
                'selectors' => [
                    '{{WRAPPER}} .ptnticker-items' => 'margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
                ],
            ]
        );
		...
		$this->add_control(
			...
			[
				...
				'type' => \Elementor\Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .owl-nav button.owl-prev i, {{WRAPPER}} .owl-nav button.owl-next i ' => 'color: {{VALUE}}',
				],
			]
		);
		...
		$this->add_group_control(
			...
			[
				...
				'types' => [ 'classic', 'gradient'],
				'selector' => '{{WRAPPER}} .owl-nav button.owl-prev i, {{WRAPPER}} .owl-nav button.owl-next i ',
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .owl-nav button.owl-prev i, {{WRAPPER}} .owl-nav button.owl-next i ' => 'width: {{SIZE}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .owl-nav button.owl-prev i, {{WRAPPER}} .owl-nav button.owl-next i ' => 'height: {{SIZE}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'size_units' => [ 'px', '%', 'em' ],
				'selectors' => [
					'{{WRAPPER}} .owl-nav button.owl-prev i, {{WRAPPER}} .owl-nav button.owl-next i ' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .owl-nav button.owl-prev i:hover, {{WRAPPER}} .owl-nav button.owl-next i:hover ' => 'width: {{SIZE}}{{UNIT}};',
				],
			]
		);
		//height
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .owl-nav button.owl-prev i:hover, {{WRAPPER}} .owl-nav button.owl-next i:hover ' => 'height: {{SIZE}}{{UNIT}};',
				],
			]
		);
	...
	protected function render(){
		...
		$sticky = ''; // added to sk
		if($ptnticker_display['ptnticker_ticker_sticky_switcher'] == 'yes'): $sticky = 'sticky'; endif;
		// =========== wp Query
		$ptnticker_post_per_page = $ptnticker_display['ptnticker_the_post_per_page'];
		$ptnticker_tags_filter = $ptnticker_display['ptnticker_the_tag_filter'];
		$ptnticker_categorys_dataa = $ptnticker_display['ptnticker_the_cat_columnsdd'];
		$ptnticker_the_blog_type = $ptnticker_display['ptnticker_the_blog_type'];
		...
		if('d' === $ptnticker_the_all_date){
			$ptnticker_coustom_day = $ptnticker_coustom_month = $ptnticker_coustom_year = $wdbp_time = '';
			$day =  gmdate('d'); 
			$wdbp_time = ['day'  => $day];
				...
				if($wp_query->have_posts()) :
					...
					echo '<div class="ptnticker-headline-box">';
						echo '<div class="ptnticker-headline-items">';
							echo '<div class="ptnticker-items"><'.esc_attr($ptnticker_display['ptnticker_title_tags']).' class="ptnticker-headline">';
								echo ('yes' === $ptnticker_display['ptnticker_title_link'])?'<a class="ptnticker-headline" href="'.esc_url(get_the_permalink()).'">'.esc_html(get_the_title()).'</a>':esc_html(get_the_title());
							echo '</'.esc_attr($ptnticker_display['ptnticker_title_tags']).'></div>';
						echo '</div>';
					echo '</div>';
					endwhile;
				endif;
